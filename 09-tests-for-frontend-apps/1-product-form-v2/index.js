...
export default class ProductForm extends BaseComponent {
  ...
  #onImageInputChange = async event => {
    ...
    if (file) {
      ...
      if (uploadedImageURL) {
        const imageElementHTML = this.#getImageItem(file.name, uploadedImageURL);
  ...
  #getImageItem(name, url) {
    return ProductForm.createElement(`
  ...
  #updateImagesList() {
    ...
    if (images) {
      ...
      const items = images.map(image => this.#getImageItem(image.source, image.url));

      const sortableList = new SortableList({
        items
      });

      imagesElement.append(sortableList.element);
    }
  }
